...
difference(){
union(){
roundedCube([pcbw+wt*2,pcbh+wt*2,5],center=true,r=5+wt,y=true,x=true,ycorners=[true,true,false,false],xcorners=[false,true,true,false],ry=[2,2,2,2],rx=[2,2,2,2]);
translate([-0.0,0,3])
roundedCube([48.5+0.5,46+wt*2,4],center=true,y=true,x=true,r=2,ycorners=[true,true,false,false],xcorners=[false,true,true,false]);
}
translate([0,0,-1])
roundedCube([pcbw+fit*2,pcbh+fit*2,5],center=true,r=5);
...
// microsd card
cutout(-pcbw/2+54-0.2,-pcbw/2+74+0.2,-pcbh/2-2,0,0,2);

// power switch
cutout(pcbw/2-5,pcbw/2+5,pcbh/2-14,pcbh/2-8,0,2);
...
translate([0,-18.5,5-0.6])
linear_extrude(2)
text("LightIvy",valign="center",halign="center",size=4,font="FreeMono:style=Bold");

// speaker
cutout(-pcbw/2+8,-pcbw/2+22,-pcbh/2+4,-pcbh/2+10,0,5);
cutout(-pcbw/2+7,-pcbw/2+23,-pcbh/2+3,-pcbh/2+11,0,2.1);
}

// speaker fill
for(i=[0:3:10])
translate([-pcbw/2+10.5+i,-pcbh/2+7,2.3])
cube([1,8,0.4],center=true);

translate([-pcbw/2+15,-pcbh/2+5.75,2.3])
cube([16,1,0.4],center=true);
translate([-pcbw/2+15,-pcbh/2+8.25,2.3])
cube([16,1,0.4],center=true);


/*translate([pcbw/2-4,pcbh/2-4,-2.5])
pin();
translate([pcbw/2-4,-pcbh/2+4,-2.5])
pin();
translate([-pcbw/2+4,pcbh/2-4,-2.5])
